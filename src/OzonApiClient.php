...
use PhpSoftBox\Ozon\Retry\DefaultRetryableRequestPolicy;
use PhpSoftBox\Ozon\Retry\NativeSleeper;
use PhpSoftBox\Ozon\Retry\OzonRetryEvent;
use PhpSoftBox\Ozon\Retry\RateLimitRetryOptions;
use PhpSoftBox\Ozon\Retry\RetryableRequestPolicyInterface;
use PhpSoftBox\Ozon\Retry\SleeperInterface;
...
use PhpSoftBox\Ozon\V1\PostingFbpV1;
...
use PhpSoftBox\Ozon\V2\PostingDigitalV2;
...
use PhpSoftBox\Ozon\V2\ReviewV2;
use PhpSoftBox\Ozon\V2\SupplyOrderV2;
use PhpSoftBox\Ozon\V2\WarehouseV2;
use PhpSoftBox\Ozon\V3\ChatV3;
use PhpSoftBox\Ozon\V3\FinanceV3;
use PhpSoftBox\Ozon\V3\PostingFboV3;
...
final class OzonApiClient
{
    ...
    public function postingFboV3(): PostingFboV3
    {
        return new PostingFboV3($this);
    }

    public function supplyOrderV2(): SupplyOrderV2
    {
        return new SupplyOrderV2($this);
    }

    public function reviewV2(): ReviewV2
    {
        return new ReviewV2($this);
    }

    public function postingDigitalV2(): PostingDigitalV2
    {
        return new PostingDigitalV2($this);
    }

    public function postingFbpV1(): PostingFbpV1
    {
        return new PostingFbpV1($this);
    }
    ...
    private function sendWithRateLimitRetry(RequestInterface $request, ?string $requestBody): ResponseInterface
    {
        ...
        for ($attempt = 1; $attempt <= $this->rateLimitMaxAttempts; $attempt++) {
            ...
            if ($this->onRateLimitRetry !== null) {
                ($this->onRateLimitRetry)(new OzonRetryEvent(
                    attempt: $attempt + 1,
                    delaySeconds: $delay,
    ...
    private function resolveProviderRetryDelay(ResponseInterface $response): ?float
    {
        $delays = [
            ...
            $this->parseNumericDelay($response->getHeaderLine('Item-Retry-After'), 60),
        ];
    ...
    private function parseNumericDelay(string $value, float $multiplier = 1): ?float
    {
        $value = trim($value);
