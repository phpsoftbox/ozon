...
final class OzonOpenApiDtoGenerator
{
    ...
    private function renderDtoClass(
        ...
        foreach ($properties as $jsonName => $propertySchema) {
            ...
            $fields[] = $this->fieldDefinition($jsonName, $propertySchema, $classNames, $schemaNamespaces, $versionNamespace, $namespace, $schemas);
        }
    ...
    private function fieldDefinition(
        ...
        $type     = $this->resolvePropertyType($schema, $classNames, $schemaNamespaces, $versionNamespace, $currentNamespace, $schemas);
    ...
    private function resolvePropertyType(
        ...
        if (isset($schema['$ref']) && is_string($schema['$ref'])) {
            ...
            $hydrator  = $this->isScalarReference($schema['$ref'], $schemas) ? 'scalarObject' : 'object';

            return [
                'phpType' => '?' . $reference['className'],
                'docType' => null,
                'hydrate' => 'OzonDtoValue::' . $hydrator . '(%s, ' . $reference['className'] . '::class)',
                'uses'    => $reference['uses'],
            ];
        }

        // В снимке Ozon встречаются массивы с items, но без явного type.
        $type = $schema['type'] ?? (isset($schema['items']) ? 'array' : null);
        if ($type === 'array') {
            $items = $schema['items'] ?? [];
            ...
            if (isset($items['$ref']) && is_string($items['$ref'])) {
                ...
                $hydrator  = $this->isScalarReference($items['$ref'], $schemas) ? 'scalarObjectList' : 'objectList';
    ...
    /** @param array<string, array<string, mixed>> $schemas */
    private function isScalarReference(string $reference, array $schemas): bool
    {
        $schema = $this->unwrapSchema($schemas[$this->refName($reference)] ?? []);
