...
final class OzonDtoValue
{
     ...
     * @return T|null
     */
    public static function object(mixed $value, string $dtoClass): ?OzonDtoInterface
    {
    ...
    /**
     * Скалярная схема (в том числе enum) представлена DTO с полем value.
     *
     * @template T of OzonDtoInterface
     * @param class-string<T> $dtoClass
     * @return T|null
     */
    public static function scalarObject(mixed $value, string $dtoClass): ?OzonDtoInterface
    {
        self::assertDtoClass($dtoClass);

        return is_scalar($value) ? $dtoClass::fromArray(['value' => $value]) : null;
    }
     ...
     * @return list<T>
     */
    public static function scalarObjectList(mixed $value, string $dtoClass): array
    {
        self::assertDtoClass($dtoClass);
        if (!is_array($value)) {
            return [];
        ...
        foreach ($value as $item) {
            if (is_scalar($item)) {
                $items[] = $dtoClass::fromArray(['value' => $item]);
            }
        }
