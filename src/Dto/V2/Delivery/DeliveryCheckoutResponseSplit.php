...
final readonly class DeliveryCheckoutResponseSplit implements OzonDtoInterface
{
    ...
    public function __construct(
        public ?DeliveryCheckoutResponseSplitCommissions $commissions,
    ...
    {
        return new self(
            commissions: OzonDtoValue::object($payload['commissions'] ?? null, DeliveryCheckoutResponseSplitCommissions::class),
            deliveryMethod: OzonDtoValue::object($payload['delivery_method'] ?? null, DeliveryCheckoutResponseSplitDeliveryMethod::class),
            deliverySchema: OzonDtoValue::scalarObject($payload['delivery_schema'] ?? null, DeliveryCheckoutResponseV2SplitDeliverySchemaEnum::class),
            items: OzonDtoValue::objectList($payload['items'] ?? null, DeliveryCheckoutResponseSplitItem::class),
            unavailableReason: OzonDtoValue::scalarObject($payload['unavailable_reason'] ?? null, DeliveryCheckoutResponseUnavailableReasonEnum::class),
            warehouseId: OzonDtoValue::int($payload['warehouse_id'] ?? null),
            extra: OzonDtoValue::extra($payload, ['commissions', 'delivery_method', 'delivery_schema', 'items', 'unavailable_reason', 'warehouse_id']),
        );
    }
}
