<?php

declare(strict_types=1);

/**
 * @generated Ozon OpenAPI DTO
 */

namespace PhpSoftBox\Ozon\Dto\V1\Posting;

use PhpSoftBox\Ozon\Dto\OzonDtoInterface;
use PhpSoftBox\Ozon\Dto\OzonDtoValue;

final readonly class PostingPostingProduct implements OzonDtoInterface
{
    /**
     * @param array<string, mixed> $extra
     */
    public function __construct(
        public ?string $currencyCode,
        public ?string $name,
        public ?string $offerId,
        public ?string $price,
        public ?int $requiredQtyForDigitalCode,
        public ?int $sku,
        public array $extra = [],
    ) {
    }

    public static function fromArray(array $payload): static
    {
        return new self(
            currencyCode: OzonDtoValue::string($payload['currency_code'] ?? null),
            name: OzonDtoValue::string($payload['name'] ?? null),
            offerId: OzonDtoValue::string($payload['offer_id'] ?? null),
            price: OzonDtoValue::string($payload['price'] ?? null),
            requiredQtyForDigitalCode: OzonDtoValue::int($payload['required_qty_for_digital_code'] ?? null),
            sku: OzonDtoValue::int($payload['sku'] ?? null),
            extra: OzonDtoValue::extra($payload, ['currency_code', 'name', 'offer_id', 'price', 'required_qty_for_digital_code', 'sku']),
        );
    }
}
